...
const hasFilters = computed(() => {
  return !!(searchParams.value.owner || searchParams.value.status || searchParams.value.type || searchParams.value.so_don)
})
</script>
    ...
    <div class="relative glass-effect rounded-2xl shadow-lg p-2">
      <div class="flex items-center gap-2">
        <div class="flex-1 relative">
          <input v-model="searchParams.q" type="text" placeholder="Nhập tên nhãn hiệu..." @keyup.enter="handleSearch"
        ...
        <button @click="handleSearch" class="btn-primary px-8 py-3">
          Tìm kiếm
        </button>
      </div>
    </div>
    ...
    <transition enter-active-class="transition duration-200 ease-out" enter-from-class="transform scale-95 opacity-0"
      ...
      <div v-if="isExpanded" class="card p-6 space-y-4 animate-slide-up">
        ...
        <div class="grid md:grid-cols-2 gap-4">
          <!-- Số đơn -->
          <div>
            <label for="so_don" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
              Số đơn
            </label>
            <input id="so_don" v-model="searchParams.so_don" type="text" placeholder="Nhập số đơn..."
              class="input-field" />
          </div>
          ...
          <div>
            ...
            <input id="owner" v-model="searchParams.owner" type="text" placeholder="Nhập tên chủ đơn..."
              class="input-field" />
          </div>
          ...
          <div>
            ...
            <select id="status" v-model="searchParams.status" class="input-field">
          ...
          <div>
            ...
            <select id="type" v-model="searchParams.type" class="input-field">
          ...
          <div>
            ...
            <input id="class" v-model="searchParams.class" type="text" placeholder="Nhập mã phân loại..."
              class="input-field" />
          </div>
        </div>
        ...
        <div v-if="hasFilters" class="pt-4 border-t border-gray-200 dark:border-gray-700">
          <div class="flex flex-wrap gap-2">
            ...
            <span v-if="searchParams.so_don"
              class="inline-flex items-center gap-1 px-3 py-1 bg-primary-100 dark:bg-primary-900/30 text-primary-700 dark:text-primary-300 rounded-full text-sm">
              Số đơn: {{ searchParams.so_don }}
              <button @click="searchParams.so_don = undefined"
                class="hover:text-primary-900 dark:hover:text-primary-100">
                <XMarkIcon class="h-3 w-3" />
              </button>
            </span>

            <span v-if="searchParams.owner"
              class="inline-flex items-center gap-1 px-3 py-1 bg-primary-100 dark:bg-primary-900/30 text-primary-700 dark:text-primary-300 rounded-full text-sm">
              Chủ đơn: {{ searchParams.owner }}
              <button @click="searchParams.owner = undefined"
                class="hover:text-primary-900 dark:hover:text-primary-100">
                <XMarkIcon class="h-3 w-3" />
              </button>
            </span>
            ...
            <span v-if="searchParams.status"
              ...
              <button @click="searchParams.status = undefined"
                class="hover:text-primary-900 dark:hover:text-primary-100">
                <XMarkIcon class="h-3 w-3" />
              </button>
            </span>

            <span v-if="searchParams.type"
              class="inline-flex items-center gap-1 px-3 py-1 bg-primary-100 dark:bg-primary-900/30 text-primary-700 dark:text-primary-300 rounded-full text-sm">
              Loại: {{typeOptions.find(t => t.value === searchParams.type)?.label}}
              <button @click="searchParams.type = undefined" class="hover:text-primary-900 dark:hover:text-primary-100">
                <XMarkIcon class="h-3 w-3" />
              </button>
            </span>

            <span v-if="searchParams.class"
              class="inline-flex items-center gap-1 px-3 py-1 bg-primary-100 dark:bg-primary-900/30 text-primary-700 dark:text-primary-300 rounded-full text-sm">
              Phân loại: {{ searchParams.class }}
              <button @click="searchParams.class = undefined"
                class="hover:text-primary-900 dark:hover:text-primary-100">
                <XMarkIcon class="h-3 w-3" />
              </button>
            </span>
