...
const toggleFavorite = async () => {
  ...
  // Use parent handler if available (from search page)
  ...
  // Use remove handler if available (from saved page) and removing
  if (handleRemoveFavorite && isFavorite.value) {
    handleRemoveFavorite(props.trademark)
    return
  }

  // Fallback to direct action (for other pages)
  saving.value = true
  try {
    if (isFavorite.value) {
...
const getStatusColor = (status: string) => {
  const colors: Record<string, string> = {
    // Vietnamese statuses from backend
...
<template>
  <div class="card overflow-hidden hover:shadow-2xl transition-all duration-300 group">
    <!-- Image -->
    <NuxtLink :to="`/trademarks/${trademark.so_don || trademark.registrationNumber || trademark.id}`" class="block relative h-48 bg-gradient-to-br from-gray-100 to-gray-200 dark:from-gray-700 dark:to-gray-800 overflow-hidden">
      <img 
        v-if="trademark.imageUrl" 
        :src="trademark.imageUrl" 
        :alt="trademark.name || 'Trademark'"
    ...
    <div class="p-6">
      <div class="flex items-start justify-between mb-4">
        <NuxtLink :to="`/trademarks/${trademark.so_don || trademark.registrationNumber || trademark.id}`" class="flex-1">
      ...
      <div class="flex items-center text-sm">
        ...
        <div v-if="trademark.so_don || trademark.registrationNumber" class="flex items-center text-sm">
          ...
          <span class="text-gray-900 dark:text-gray-100 font-mono text-xs">{{ trademark.so_don || trademark.registrationNumber }}</span>
        </div>
      </div>

      <div class="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
        <NuxtLink
          :to="`/trademarks/${trademark.so_don || trademark.registrationNumber || trademark.id}`"
